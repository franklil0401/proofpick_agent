...
产品首页提供五个固定场景。**Trusted Mode · Stable 是默认模式**；**Online Research · Experimental** 必须显式确认。离线页面始终显示“这是固定的脱敏结果回放，不是实时模型调用。”；回放数据来自已保存的公开测试结果或当前治理数据，不包含 Prompt、凭据、私人路径和隐藏思维链。在线失败会显示 `online_unavailable`/unknown，不会偷偷切换或伪装成功。
...
访问 `http://127.0.0.1:8088/app.html`。在线 Trusted 模式需先按 [RC3 Windows 复现说明](v2_9h_windows_reproduction.md)启动完整服务；Open Research 是 Beta，UI 会先展示边界并要求确认，不会静默调用公网。
...
| 2 数据库外开放研究（Experimental） | “研究治理库外 AirPods Max 2 爱尔兰官方页面的形态、降噪、通透、空间音频和续航。”；Source Search → Extractor → Temporary Evidence → Open Report | 回放保存的真实 Apple IE 页面验证；`trusted_eligible=false`，进入 Checker 为 0 | 现场默认使用 `?demo=open-airpods-max` 脱敏回放。真实脚本会调用搜索 API，只有明确确认费用与 Beta 边界后才运行；失败只输出 degraded/unknown |
...
Online Research 不保证稳定找到官方地区页或抓取所有网站，不宣称实时价格、库存或全市场覆盖。历史独立首次实际取证为 `0/15`，V2-9F exposed regression 为 `6/15`，V2-9G 未接入生产的 Playwright 保守投影为 `7/15`；安全返回 unknown 不算完成网页研究。
