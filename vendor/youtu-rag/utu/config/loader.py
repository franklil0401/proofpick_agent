...
class ConfigLoader:
    ...
    def load_toolkit_config(cls, name: str = "search") -> ToolkitConfig:
        ...
        Tries to load from:
        1. configs/rag/rag_tools/ (for RAG-specific toolkits like kb_search)
        2. configs/tools/ (for general toolkits)
        """
        # Try RAG tools path first (for kb_search and other RAG-specific tools)
        try:
            cfg = cls._load_config_to_dict(name, config_path="../../configs/rag/rag_tools")
            return ToolkitConfig(**cfg)
        except Exception:
            # Fall back to general tools path
