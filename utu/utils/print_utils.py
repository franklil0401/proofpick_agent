...
class PrintUtils:
    ...
    def print_and_stream_tool(
        ...
        """同时输出到终端和流式队列的工具方法"""
        # 1. 终端输出
        PrintUtils.print_tool(msg, color=color, add_prefix=add_prefix, prefix=prefix, end=end, flush=flush)
        ...
        if event_queue is not None:
            ...
            tool_log_event = SimpleNamespace(
                type="custom.tool_log",
                message=msg,
                tool_name=tool_name,
                timestamp=None
            )
            
            event_queue.put_nowait(RawResponsesStreamEvent(data=tool_log_event))
